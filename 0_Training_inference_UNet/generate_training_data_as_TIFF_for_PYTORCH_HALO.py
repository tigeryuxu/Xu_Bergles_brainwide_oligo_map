...
list_folder = ['/media/user/4TB_SSD/Training_UNet_HALO/halo/For_training/']
...
for input_path in list_folder:
    ...
    images = glob.glob(os.path.join(input_path,'*_CONGO_im.tif'))    # can switch this to "*truth.tif" if there is no name for "input"
    images.sort(key=natsort_keygen(alg=ns.REAL))  # natural sorting
    examples = [dict(input=i,truth=i.replace('_CONGO_im.tif','_halo_labels.tif'), 
                     autofluor=i.replace('_CONGO_im.tif','_red_im.tif'), 
                     myelin=i.replace('_CONGO_im.tif', '_green_im.tif')) for i in images]
    ...
    for ex in examples:
            ...
            red_im = np.asarray(tiff.imread(ex['autofluor']), dtype=np.uint16)
            # myelin_im = np.asarray(tiff.imread(ex['myelin']), dtype=np.uint16)
            
            ### Sometimes has 4 dimensions???
            red_im = np.squeeze(red_im)
            ...
            if os.path.exists(seg_name):
                ...
                print('No labels found, assuming blank labels data')
                seg_im = np.zeros(np.shape(input_im))
                
                
            seg_im = np.asarray(seg_im, dtype=np.uint8)
            ...
            ### *** DOESNT WORK CURRENTLY BECAUSE WE ARE ADDING IN DATA WITH ONLY VALUE OF 1
            ...
            tiff.imsave(input_path + filename + '_block_TRUTH.tif', np.uint8(truth_im))
            ...
            for z in range(0, depth_im + quad_depth, round(quad_depth - quad_depth * overlap_percent)):
                ...
                for x in range(0, width + quad_size, round(quad_size - quad_size * overlap_percent)):
                    ...
                    for y in range(0, height + quad_size, round(quad_size - quad_size * overlap_percent)):
                          ...
                          quad_red = red_im[z:z + quad_depth, x:x + quad_size, y:y + quad_size]
                          # quad_myelin = myelin_im[z:z + quad_depth, x:x + quad_size, y:y + quad_size]
                          ...
                          tiff.imsave(sav_dir + filename + str(int(x)) + '_' + str(int(y)) + '_' + str(int(z)) +'_block_RED.tif', np.uint16(quad_red))
                          # tiff.imsave(sav_dir + filename + str(int(x)) + '_' + str(int(y)) + '_' + str(int(z)) +'_block_MYELIN.tif', np.uint16(quad_myelin))
