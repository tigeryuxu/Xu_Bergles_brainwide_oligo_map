...
# list_brains = get_metadata(mouse_num = ['M260', 'M147', 'M286', 'M271', 'Otx6', 'M265', 'M312'])  ### for aging


list_brains = get_metadata(mouse_num = ['M260', 'M271', 'Otx6', 'M286'])  ### for aging
...
sav_fold = '/media/user/8TB_HDD/Plot_SLICES_FULL/'
...
for fold in list_brains:
    ...
    def save_2D_slice_low_res(density_map, ref_bounds, ref_atlas, keys_df, reg_name, sav_dir, exp_name, vmin, vmax, axis=0):
        ...
        if axis == 0 and reg_name == 'Secondary motor area ':
            mid_slice = 148
            print('Preset region')            
        ...
        if axis==0:
            slice_im = density_map[mid_slice, ...] 
            ### also upscale atlas to allow masking out
            tmp_atlas_COLOR = np.asarray(tmp_atlas_COLOR, dtype=np.uint32)
            crop_down = tmp_atlas_COLOR[mid_slice, ...]
            crop_bounds = ref_bounds[mid_slice, ...]
            
            crop_ref = ref_atlas[mid_slice, ...]
        ...
        tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_FULL_atlas_slice_' + str(mid_slice) + '_AXIS_'  + str(axis) + '.tif', crop_ref)
        ...
        tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_DENSITY_MAP_2D_slice_ATLAS_MASKED_' + str(mid_slice) + '_AXIS_'  + str(axis) + '.tif', crop_down)
        
        
        ### Create boundary image for reference atlas

        bw_slice = find_boundaries(crop_ref)
        bw_slice = np.asarray(bw_slice, dtype=np.uint8)
        
        # plt.figure(); plt.imshow(bw_slice, cmap='Greys_r')
        # plt.axis('off');
        ...
        tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_DENSITY_MAP_2D_slice_BOUNDS_FULL_' + str(mid_slice) + '_AXIS_'  + str(axis) + '.tif', bw_slice)
        ...
        plt.figure(); plt.imshow(slice_im, cmap='turbo', 
                                 vmin=vmin, vmax=vmax)
        plt.axis('off');
        plt.savefig(sav_dir + exp_name + '_' + reg_name + '_DENSITY_MAP_2D_slice_FULL_' + str(mid_slice) + '_AXIS_'  + str(axis) + '.png', dpi=300, bbox_inches='tight')
        tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_DENSITY_MAP_2D_slice_FULL_' + str(mid_slice) + '_AXIS_'  + str(axis) + '.tif', slice_im)
               
        
        

    
    # save_2D_slice_low_res(density_map, ref_bounds, ref_atlas=ref_atlas, keys_df=keys_df, 
    #                        reg_name='Retrosplenial area', exp_name=fold['name'],
    #                         sav_dir=sav_fold, axis=1, vmin=0, vmax=30000)

    # save_2D_slice_low_res(density_map, ref_bounds, ref_atlas=ref_atlas, keys_df=keys_df, 
    #                        reg_name='Ectorhinal area', exp_name=fold['name'],
    ...
    save_2D_slice_low_res(density_map, ref_bounds, ref_atlas=ref_atlas, keys_df=keys_df, 
                           reg_name='Secondary motor area', exp_name=fold['name'],
                            sav_dir=sav_fold, axis=1, vmin=0, vmax=30000)
    ...
    ### REMOVE PADDING
    right_nopad = atlas[z_after:-z_after, x_after:-x_after, x_after:-x_after]
    ...
    def save_2D_slice_high_res(dset, atlas, keys_df, reg_name, sav_dir, exp_name, down_factor, axis=0):
        ...
        if axis == 1 and reg_name == 'Secondary motor area ':
            mid_slice = 148
            print('Preset region')            
        ...
        if axis==0:
            ...
            atlas_slice = atlas[mid_slice, ...]
            
        
        if axis==1:
            slice_im = dset[:, mid, :] 
            
            ### need to scale to isotropic
            slice_im = rescale(slice_im, [down_factor[1]/down_factor[0], 1], order=1, preserve_range=True)
        ...
        tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_EXTRACTED_2D_slice_FULL_' + str(mid) + '_AXIS_'  + str(axis) + '.tif', slice_im)
        
        
        mask_rescaled = resize(crop_down, slice_im.shape, order=0)     
        tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_EXTRACTED_2D_ATLAS_MASK_' + str(mid) + '_AXIS_'  + str(axis) + '.tif', np.asarray(mask_rescaled, dtype=np.uint32))
        
         
        full_atlas = resize(atlas_slice, slice_im.shape, order=0)         
        tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_EXTRACTED_2D_ATLAS_FULL_' + str(mid) + '_AXIS_'  + str(axis) + '.tif', np.asarray(full_atlas, dtype=np.uint32))
        
        
        
        ### also mask out surroundings
        # slice_im[mask_rescaled == 0] = 0
        # tiff.imwrite(sav_dir + exp_name + '_' + reg_name + '_EXTRACTED_2D_MASKED_FULL_' + str(mid) + '_AXIS_'  + str(axis) + '.tif', mask_rescaled)
    ...
    save_2D_slice_high_res(dset, atlas=right_nopad, keys_df=keys_df, reg_name='Secondary motor area', 
                            sav_dir=sav_fold, exp_name=fold['name'], down_factor=down_factor, axis=0)
